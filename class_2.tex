...
\begin{definition}[Lower Bound]
    Let $(A,\sqsubseteq)$ be a poset. Then (i) $x$ in $A$ is an \emph{lower bound} of a subset $B$ of $A$ if for all $y$ in $B$, it holds that $y \sqsubseteq x$; $x$ is the \emph{greatest lower bound} (glb) of $B$ if $x$ is a lower bound of $B$ and for all lower bounds $y$ of $B$, we have $y \sqsubseteq x$. %We denote such $x$ by $\bigsqcap B$.
\end{definition}
